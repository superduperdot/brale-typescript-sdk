...
class AddressesService {
    ...
    async createExternal(accountId, request) {
        ...
        const idempotencyKey = (0, idempotency_1.createAddressIdempotencyKey)({
            accountId,
    ...
    validateAddressId(addressId) {
        if (!addressId || typeof addressId !== 'string' || addressId.trim().length === 0) {
            throw new api_error_1.BraleAPIError('Invalid address ID', 400, 'INVALID_ADDRESS_ID');
        }
    }
    /**
    ...
    validateAddressString(address) {
        if (!address || typeof address !== 'string' || address.trim().length === 0) {
            throw new api_error_1.BraleAPIError('Invalid address', 400, 'INVALID_ADDRESS');
        }
        // Basic length validation (most blockchain addresses are 20-100 characters)
        if (address.length < 20 || address.length > 100) {
            throw new api_error_1.BraleAPIError('Address length invalid', 400, 'INVALID_ADDRESS_LENGTH');
        }
    }
    /**
    ...
    validateCreateExternalAddressRequest(request) {
        ...
        if (!request.network) {
            throw new api_error_1.BraleAPIError('Network is required', 400, 'MISSING_NETWORK');
        }
        if (request.label && request.label.length > 100) {
            throw new api_error_1.BraleAPIError('Label too long (max 100 characters)', 400, 'LABEL_TOO_LONG');
        }
    }
    /**
    ...
    validateUpdateAddressRequest(request) {
        if (request.label !== undefined) {
            if (typeof request.label !== 'string') {
                throw new api_error_1.BraleAPIError('Invalid label', 400, 'INVALID_LABEL');
            }
