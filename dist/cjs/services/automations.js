...
class AutomationsService {
    ...
    async create(accountId, request) {
        ...
        const idempotencyKey = (0, idempotency_1.generateIdempotencyKey)('automation');
    ...
    async trigger(accountId, automationId) {
        this.validateAccountId(accountId);
        this.validateAutomationId(automationId);
        const idempotencyKey = (0, idempotency_1.generateIdempotencyKey)('trigger');
    ...
    validateAutomationId(automationId) {
        if (!automationId || typeof automationId !== 'string' || automationId.trim().length === 0) {
            throw new api_error_1.BraleAPIError('Invalid automation ID', 400, 'INVALID_AUTOMATION_ID');
        }
    }
    /**
    ...
    validateCreateAutomationRequest(request) {
        if (!request.name || typeof request.name !== 'string' || request.name.trim().length === 0) {
            throw new api_error_1.BraleAPIError('Automation name is required', 400, 'MISSING_AUTOMATION_NAME');
        }
        if (request.name.length > 100) {
            throw new api_error_1.BraleAPIError('Automation name too long (max 100 characters)', 400, 'NAME_TOO_LONG');
        }
        if (!request.type) {
            throw new api_error_1.BraleAPIError('Automation type is required', 400, 'MISSING_AUTOMATION_TYPE');
        }
        if (!request.triggers || request.triggers.length === 0) {
            throw new api_error_1.BraleAPIError('At least one trigger is required', 400, 'MISSING_TRIGGERS');
        }
        if (!request.actions || request.actions.length === 0) {
            throw new api_error_1.BraleAPIError('At least one action is required', 400, 'MISSING_ACTIONS');
        }
    }
    /**
    ...
    validateUpdateAutomationRequest(request) {
        if (request.name !== undefined) {
            if (typeof request.name !== 'string' || request.name.trim().length === 0) {
                throw new api_error_1.BraleAPIError('Invalid automation name', 400, 'INVALID_AUTOMATION_NAME');
            }
            if (request.name.length > 100) {
                throw new api_error_1.BraleAPIError('Automation name too long (max 100 characters)', 400, 'NAME_TOO_LONG');
            }
