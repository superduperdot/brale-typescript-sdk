...
exports.BraleAuth = void 0;
const axios_1 = __importDefault(require("axios"));
const api_error_1 = require("./errors/api-error");
const credential_validator_1 = require("./security/credential-validator");
const credential_rotation_1 = require("./security/credential-rotation");
/**
...
class BraleAuth {
    ...
    constructor(config) {
        ...
        const validation = credential_validator_1.CredentialValidator.validateCredentials(config.clientId, config.clientSecret);
        ...
        const exposureRisks = credential_validator_1.CredentialValidator.detectCredentialExposure(config);
        ...
        this.httpClient.interceptors.response.use((response) => response, (error) => {
            if (axios_1.default.isAxiosError(error)) {
                if (error.response) {
                    throw api_error_1.BraleAuthError.fromResponse(error.response.status, error.response.data, error.response.headers['x-request-id']);
                }
                throw new api_error_1.BraleNetworkError(`Network error during authentication: ${error.message}`, error);
            }
            throw error;
        });
    }
     ...
     */
    createAuthenticatedClient(baseURL, timeout) {
    ...
    async revokeToken() {
        if (this.storedToken) {
            try {
                ...
                await this.httpClient.post('/oauth2/revoke', `token=${this.storedToken.accessToken}`, {
                    headers: {
    ...
    async fetchNewToken() {
        ...
        const credentials = Buffer.from(`${this.config.clientId}:${this.config.clientSecret}`).toString('base64');
        try {
            const response = await this.httpClient.post('/oauth2/token', 'grant_type=client_credentials', {
                headers: {
                    'Authorization': `Basic ${credentials}`,
                    'Content-Type': 'application/x-www-form-urlencoded',
                }
            ...
            this.storedToken = null;
            if (error instanceof api_error_1.BraleAuthError) {
                throw new api_error_1.BraleAuthError(`Failed to authenticate with Brale API: ${error.message}`, { clientId: this.config.clientId });
            }
            throw error;
        }
    }
     ...
     */
    enableCredentialRotation(rotationProvider, config) {
        this.rotationManager = new credential_rotation_1.CredentialRotationManager(config);
        ...
        this.rotationManager.on('rotation_event', (event) => {
