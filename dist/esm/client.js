    ...
    setupResponseInterceptor() {
        this.httpClient.interceptors.response.use((response) => response, (error) => {
            if (error.response) {
                ...
                throw BraleAPIError.fromResponse(error.response.status, error.response.data, error.response.headers['x-request-id']);
            }
            else if (error.request) {
                // Network error
                throw new BraleNetworkError(`Network error: ${error.message}`, error);
            }
            else {
                // Other error
                throw error;
