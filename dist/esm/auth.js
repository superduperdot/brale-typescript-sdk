...
import { CredentialRotationManager } from './security/credential-rotation';
/**
    ...
    constructor(config) {
        ...
        this.httpClient.interceptors.response.use((response) => response, (error) => {
            if (axios.isAxiosError(error)) {
                if (error.response) {
                    throw BraleAuthError.fromResponse(error.response.status, error.response.data, error.response.headers['x-request-id']);
                }
                throw new BraleNetworkError(`Network error during authentication: ${error.message}`, error);
            }
            throw error;
        });
    }
    ...
    async fetchNewToken() {
        ...
        try {
            ...
            if (error instanceof BraleAuthError) {
                throw new BraleAuthError(`Failed to authenticate with Brale API: ${error.message}`, { clientId: this.config.clientId });
            }
            throw error;
        }
    }
