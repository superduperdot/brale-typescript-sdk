...
export function validatePaginationParams(params: PaginationParams = {}): Required<PaginationParams> {
  const validated: Required<PaginationParams> = {
 ...
 */
export function createPaginationQuery(params: PaginationParams): Record<string, string> {
...
export class PaginatedIterator<T> {
  private currentParams: Required<PaginationParams>;
 ...
 */
export function createAutoPaginator<T>(
  ...
  return {
     ...
     */
    all: async (params?: PaginationParams, maxPages?: number): Promise<T[]> => {
